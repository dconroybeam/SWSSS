...
R is really good at analyzing data. It’s also really useful for manipulating data: combining, labeling, rearranging, repeating, etc. Below are a number of functions commonly used to manage data.
...
Creating agent-based models requires generating data that mimics real-world processes. R handily has many built-in functions for generating simulated data.
...
flips<-rbinom(numberOfCoins,numberOfFlips,probabilityOfHeads)
...
```{r}
#Simulations draws from a uniform distribution
#Good for simulating dice rolls

rolls<-runif(numberOfDice,minValue,maxValue)
rolls<-runif(1,1,6)
rolls<-runif(100,10,909835)
...
```{r}
#Simulates draws from a normal distribution
#Good for simulating all kinds of psychological variables
