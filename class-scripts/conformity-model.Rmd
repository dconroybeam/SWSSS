...
for(m in 1:months){
  ...
  #Store the proportion of agents wearing each facial hair style at each month
  results$prop[results$time==m &
                 results$style=="beard"]<-mean(agents$facialHair=="beard")
  results$prop[results$time==m & 
                 results$style=="clean"]<-mean(agents$facialHair=="clean")
  results$prop[results$time==m &
                 results$style=="stash"]<-mean(agents$facialHair=="stash")
...
p1<-ggplot(data=results,
  ...
  scale_fill_manual(labels=c("Beard","Clean-Shaven","Moustache"),
                    values=c("sienna","grey75","forestgreen"))+
  geom_area()
