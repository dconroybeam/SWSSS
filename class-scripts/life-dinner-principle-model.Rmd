...
encounter<-function(foxes,rabbits){
  ...
  #Returns 1 if the fox outruns the rabbit; 0 otherwise
...
results<-data.frame("loop"=rep(1:modelLoops,each=generations*2),
...
for (m in 1:modelLoops) {
  ...
  for (g in 1:generations) {
    ...
    for (e in 1:nEncounters) {
      ...
      #Determine which rabbits each fox encounters
      ...
      #Determine whether each encountered rabbit gets caught
      caught <-
        encounter(foxes, rabbits[match(encountered, rabbits$ID), ])
      
      #Update the eaten status for encountered rabbits
      rabbits$eaten[match(encountered, rabbits$ID)] <- caught
      
      #Update the catches status for each fox
